...
- `start-local.sh` uses `JAVA_BIN`, not `JAVA_HOME`. If the default binary is absent, use `JAVA_BIN=/usr/lib/jvm/temurin-8-jdk-amd64/bin/java` when that blueprint-provisioned binary exists. Use the corresponding `JAVA_HOME` for Maven builds.
...
- The statistics screen computes charts client-side from account data; rendering it does not itself prove the `/statistics/**` gateway route. Separately report which API routes browser traffic actually exercised. Skipping reminder signup likewise leaves `/notifications/**` untested.
...
- For gateway UI migrations, passively capture request methods/URLs, status codes, MIME types, completion events, and Authorization-presence booleans. Never retain OAuth bodies or header values. Same-origin bearer-token UI testing does not prove cross-origin preflight or cookie forwarding.
